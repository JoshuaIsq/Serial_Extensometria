...
from PySide6.QtWidgets import (QApplication, QLabel, QMainWindow, QToolBar, QCheckBox, 
                               QPushButton,  QWidget, QVBoxLayout, QListWidget, QStackedWidget, QLabel)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.channel_bar = QToolBar("Canais vísiveis", self)
        self.channel_bar.setMovable(False)
        self.addToolBar(self.channel_bar)
        ...
        self.paused = False
        self.pause_button = QPushButton("Pausar Gráfico")
    ...
    def atualizar_grafico(self):
        ...
        for i, curva in enumerate(self.curvas):
            curva.setData(
                tempos,
                list(self.valores_canais[i]),
                connect="finite",
            )

        # Faz o eixo do tempo acompanhar os dados.
        fim = max(10, tempos[-1])
    ...
    def navigate(self):
        ...
        #Menu Lateral
        self.lateral_menu = QListWidget()
        self.lateral_menu.setFixedWidth(180)
        self.lateral_menu.addItems(["Aquisição", "Configurações", "Em breve"])

        #Guarda as paginas em um QStackedWidget para alternar entre elas
        self.paginas = QStackedWidget()

        #Pagina principal
        self.pagina_aquisicao = PaginaAquisição(self.grafico)
        ...
        self.lateral_menu.currentRowChanged.connect(self.paginas.setCurrentIndex)

        self.lateral_menu.setCurrentRow(0)
